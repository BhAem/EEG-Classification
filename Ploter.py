...
def plot_save_Result(final_acc_list, model_name, dataset='Benchmark', UD=0, ratio=1, win_size='1', text=True):
    ...
    df.to_csv(f'./Result/{dataset}/{model_name}/{proportion}_{val_way}_Classification_Result({win_size}S).csv',
              index=False)
    # df.to_csv(f'/cache/Result/{dataset}/{model_name}/{proportion}_{val_way}_Classification_Result({win_size}S).csv', index=False)
    ...
    # plt.savefig(f'/cache/Result/{dataset}/{model_name}/{proportion}_{val_way}_Classification_Result({win_size}S).png')
    plt.show()